...
// Minor changes to make it work -- ELP 8/3/09
// Used in TestStyleCPP
...
class Properties {
  ...
  public function setProperty($key, $value) {
    Properties_Section_Property::_testKey($key); // added _  ELP 8/3/09
    Properties_Section_Property::_testValue($key); // added _  ELP 8/3/09
  ...
  public function store($file, $header = null) {
    ...
    $data .= $this->toString();
    // write
    if (is_resource($file) && get_resource_type($file) === 'file') { // added check is_resource ELP 8/3/09
      fwrite($file, $data);
    }
    else {
...
class Properties_Section_Property implements Properties_ISection {
   ...
   * @throw InvalidArgumentException
   */
  public static function _testKey($key) { // made public ELP 8/3/09
    if (!isset($key)) {
   ...
   * @throw InvalidArgumentException
   */
  public static function _testValue($value) { // maded public ELP 8/3/09
    if (!isset($value)) {
...
function testProperties() {
    ...
    $p = new Properties();
    // Read from string (from array and file handle is possible too):
    $p->load(file_get_contents('testfiles/test.properties'));
    ...
    $a = $p->getProperty('a');
    assert('$a == "a string"');
    $d = $p->getProperty('d.e.f');
    assert('$d == "another string"');
    ...
    // Save properties method 1:
    $p->store('testfiles/testout.properties'); // gives warning

    // Save properties method 2:
    file_put_contents('testfiles/testout2.properties', $p->toString());

    // Save properties method 3 (includes comments and blanks):
    file_put_contents('testfiles/testout3.properties', $p->toString(true));
