...
void Product::print() const {
    cout << fixed << setprecision(2); // magic formula
    cout << setw(NAME_WIDTH) << left << name
         << setw(PRICE_WIDTH) << right << price
         << setw(QTY_WIDTH) << quantity
         << setw(VALUE_WIDTH) << getValue() << endl;
}

string Product::toString() {
    stringstream sstr;
    sstr << setprecision(2) << fixed; // magic formula
    sstr << setw(NAME_WIDTH) << left << name
         << setw(PRICE_WIDTH) << right << price
         << setw(QTY_WIDTH) << quantity
         << setw(VALUE_WIDTH) << getValue() << endl;
